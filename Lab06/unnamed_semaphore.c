...
int main(int argc, char *argv[]) {
	...
	if((shmId = shmget(IPC_PRIVATE, SIZE + sizeof(sem_t), IPC_CREAT|S_IRUSR|S_IWUSR)) < 0) {
		perror("shmget\n");
		exit(1);
	}
	...
	// Create an unnamed mutual exclusion semaphore, semMutex with an initial value 1
	if (sem_init(&semMutex, shmId, 1) == -1) {
		perror ("sem_init\n"); 
		exit (1);
	}
